...
const serverSideDir = path.join(ppStuffDir, 'benchmark-silent-spring');;
const reportPath = path.join(ppStuffDir, 'raw-data', 'benchmark-silent-spring.odgen.md');
...
async function main() {
  ...
  async function Analyze(libDir) {
    try {
      ...
      // python3 /root/projs/ODGen/odgen.py /root/projs/prototype-pollution-stuff/server-side-libs/bmoor_lib -ma -t proto_pollution --timeout 180 2>&1 | tee bmoor1.out.txt
      const odgen = spawn(
        'python3', [
          odgenPath, 
